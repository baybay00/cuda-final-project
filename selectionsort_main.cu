...
int main(int argc, char* argv[])
{
    ...
    for(int i = 0; i < in_elements - i; i++)
    {
        selectionsort_kernel<<<dim_grid, dim_block>>>(in_d, in_elements * sizeof(float), i);
        cuda_ret = cudaDeviceSynchronize();
        if(cuda_ret != cudaSuccess) printf("Unable to launch kernel in pass: %d", i);
    }
