...
#include "support.h"
#include "insertionsort_kernel.cu"
...
int main(int argc, char* argv[])
{
    ...
    if(argc == 1) {
        ...
        printf("\n    Invalid input parameters!"
           "\n    Usage: ./insertion_sort          # Input of size 100,000 is used"
           "\n    Usage: ./insertion_sort <m>      # Input of size m is used"
           "\n");
        exit(0);
    }
    ...
    // Timing host-side insertion sort ---------------------------------------
    printf("Running host-side insertion sort...\n"); fflush(stdout);
    startTime(&timer);
    ...
    stopTime(&timer); 
    printf("Host-side insertion sort time: %f s\n", elapsedTime(timer));
    ...
    insertionsort_kernel<<<dim_grid, dim_block>>>(in_d, in_elements * sizeof(float));
    cuda_ret = cudaDeviceSynchronize();
    if(cuda_ret != cudaSuccess) printf("Unable to launch kernel");

    stopTime(&timer); 
    printf("Device insertion sort time: %f s\n", elapsedTime(timer));
